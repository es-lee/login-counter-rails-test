...
class UsersControllerTest < ActionController::TestCase
  ...
  test '#signup_failure_user_longer' do
    post :signup, {'username' => 'testieeeeeeeeeeeeeeeeeeeeeeeeeeeeee', 'password' => 'testpass'}
    json = JSON.parse(response.body)
    assert json['error_code'] == -1
  end
  ...
  test '#signup_failure_pwd_longer' do
    post :signup, {'username' => 'testuser', 'password' => 'testersseagsefsgefsgesgesgefsgsersgesr'}
    json = JSON.parse(response.body)
    assert json['error_code'] == -2
  end
